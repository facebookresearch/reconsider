...
class Transformer(nn.Module):
    ...
    def forward(self, x, attn_mask=None, head_mask=None,
                graph_encodings=None, rel_encodings=None,
                hop_linears=None, start_graph_encodings=None):
        """
        Parameters
        ----------
        x: torch.tensor(bs, seq_length, dim)
        ...
        if graph_encodings is not None:
            N, M = graph_encodings.size(0), graph_encodings.size(1)
        ...
        for i, layer_module in enumerate(self.layer):
            ...
            if graph_encodings is not None and i>=start_graph_encodings:
                switch_encodings = hidden_state[:,0,:].contiguous()
                switch_encodings = self.dropout(switch_encodings)
                switch_encodings = fusion_modeling(switch_encodings.view(N, M, -1),
                                               rel_encodings,
                                               graph_encodings,
                                               hop_linears[i-start_graph_encodings]).view(N*M, 1, -1)
                hidden_state = torch.cat([switch_encodings, hidden_state[:,1:,:]], 1)
...
class DistilBertModel(DistilBertPreTrainedModel):
    ...
    def forward(self,
                ...
                graph_encodings=None, rel_encodings=None, hop_linears=None, start_graph_encodings=None):
        ...
        tfmr_output = self.transformer(x=embedding_output,
                                       ...
                                       rel_encodings=rel_encodings,
                                       hop_linears=hop_linears,
                                       start_graph_encodings=start_graph_encodings)
...
class DistilBertForQuestionAnswering(DistilBertPreTrainedModel):
    ...
    """
    def __init__(self, config, device, n_classes=2, loss_type='mml', variant_id=0, tau=0):
        super(DistilBertForQuestionAnswering, self).__init__(config)

        self.bert = DistilBertModel(config)
        self.qa_outputs = nn.Linear(config.dim, 2)
        self.qa_classifier = nn.Linear(config.dim, n_classes)
